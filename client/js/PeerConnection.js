...
(function (w, $) {
  ...
  CA.PeerConnection.prototype.handleStreamerAnswer = function (params) {
      ...
      var mgSdp = this._prepareUplinkOffer(params, mediaDirection, sdp);
      mgSdp.flush();
      ...
      // TODO: what to do with RTCP RRs generated by downlink receivers with random SSRC?
      // what if they collide with existing scope SSRC?
      
      function onLocalDescriptorSet() {
        var answerSdp;
        if (uplink) {
          answerSdp = self._prepareUplinkAnswer(params, offerSdp);
        } else {
          answerSdp = self._prepareDownlinkAnswer(params, {type: 'offer', sdp: params.offerSdp})
        }
  ...
  CA.PeerConnection.prototype._prepareUplinkOffer = function (params, mediaDirection, offerSdp) {
      var mgSdp = new ManageableSDP(offerSdp);
      // set values in offer adjusted by relay
      ...
      mgSdp.mediaSections[0].direction = mediaDirection;
      mgSdp.mediaSections[1].direction = mediaDirection;
      return mgSdp;
  }
  ...
  CA.PeerConnection.prototype._setStreamerEndpointParams = function (manageableSdp, params) {
    ...
    manageableSdp.mediaSections[0].attributes['ice-options'] = 'trickle';
    manageableSdp.mediaSections[1].attributes['ice-options'] = 'trickle';

    // the only candidate with public relay's IP and fixed port
    manageableSdp.mediaSections[0].attributes['candidate'] = params.candidate;
    manageableSdp.mediaSections[1].attributes['candidate'] = params.candidate;
    
    manageableSdp.mediaSections[0].port = params.port;
    manageableSdp.mediaSections[1].port = params.port;

    // todo: add to parser
    //manageableSdp.mediaSections[0].attributes['c'] = 'IN IP4 ' + params.address;
    //manageableSdp.mediaSections[1].attributes['c'] = 'IN IP4 ' + params.address;

    manageableSdp.mediaSections[0].rtcp.addrInfo = 'IN IP4 ' + params.address;
    manageableSdp.mediaSections[0].rtcp.port = params.port;
    manageableSdp.mediaSections[1].rtcp.addrInfo = 'IN IP4 ' + params.address;
    manageableSdp.mediaSections[1].rtcp.port = params.port;

    // ICE parameters generated on relay side; same values for both a/v for bundled connection:
    manageableSdp.mediaSections[0].attributes['ice-ufrag'] = params.iceUfrag;
    manageableSdp.mediaSections[0].attributes['ice-pwd'] = params.icePwd;
    manageableSdp.mediaSections[1].attributes['ice-ufrag'] = params.iceUfrag;
    manageableSdp.mediaSections[1].attributes['ice-pwd'] = params.icePwd;
  }
